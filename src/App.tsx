...
type NarrativeLens = 'all' | 'covenant' | 'civilizations' | 'church' | 'america';
...
const lensOptions: Array<{ id: NarrativeLens; label: string; description: string }> = [
  { id: 'all', label: 'Full Canon', description: 'Trace the complete story from Creation to the present age.' },
  { id: 'covenant', label: 'Covenant Arc', description: 'Follow covenant milestones and redemptive turning points.' },
  { id: 'civilizations', label: 'Civilizations', description: 'Focus on empires, institutions, and cultural formation.' },
  { id: 'church', label: 'Church Story', description: 'Track Christendom, doctrine, and reform through history.' },
  { id: 'america', label: 'American Legacy', description: 'Study the U.S. founding and modern liberty struggles.' },
];
...
const lensPeriodIds: Record<Exclude<NarrativeLens, 'all'>, string[]> = {
  ...
  church: ['life-of-christ', 'early-church', 'church-fathers', 'constantinian-era', 'middle-ages', 'reformation', 'modern-era'],
...
function buildSearchText(period: HistoricalPeriod): string {
  return [
    ...
    period.quote ?? '',
    period.quoteSource ?? '',
    period.keyFigures.map((figure) => `${figure.name} ${figure.title ?? ''} ${figure.significance}`).join(' '),
    period.keyEvents.map((event) => `${event.name} ${event.date} ${event.description} ${event.location ?? ''}`).join(' '),
  ]
    .join(' ')
    .toLowerCase();
}
...
function intensityScore(period: HistoricalPeriod): number {
  return period.keyEvents.length * 3 + period.keyFigures.length * 2 + period.sources.length + (period.quote ? 1 : 0);
}

function estimateReadMinutes(period: HistoricalPeriod): number {
  const words = period.description.split(/\s+/).length + period.keyEvents.reduce((count, event) => count + event.description.split(/\s+/).length, 0);
...
export default function App() {
  ...
  const activeLensMeta = lensOptions.find((lens) => lens.id === activeLens) ?? lensOptions[0];
  ...
  const filteredPeriods = useMemo(() => {
    ...
    if (sortMode === 'canonical') {
      return lensFiltered.sort((a, b) => (canonicalIndexMap.get(a.id) ?? 0) - (canonicalIndexMap.get(b.id) ?? 0));
    }
    ...
    return lensFiltered.sort((a, b) => estimateReadMinutes(a) - estimateReadMinutes(b));
  ...
  const selectedInFiltered = !!selectedPeriod && filteredPeriods.some((period) => period.id === selectedPeriod.id);

  const detailSequence = selectedInFiltered ? filteredPeriods : historicalPeriods;
  const selectedDetailIndex = selectedPeriod ? detailSequence.findIndex((period) => period.id === selectedPeriod.id) : -1;
  const hasPreviousDetail = selectedDetailIndex > 0;
  const hasNextDetail = selectedDetailIndex >= 0 && selectedDetailIndex < detailSequence.length - 1;

  const totalVisibleEvents = filteredPeriods.reduce((sum, period) => sum + period.keyEvents.length, 0);
  const totalVisibleFigures = filteredPeriods.reduce((sum, period) => sum + period.keyFigures.length, 0);
  const estimatedVisibleMinutes = filteredPeriods.reduce((sum, period) => sum + estimateReadMinutes(period), 0);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (viewMode === 'detail' && event.key === 'ArrowLeft' && hasPreviousDetail) {
        event.preventDefault();
      ...
      if (viewMode === 'detail' && event.key === 'ArrowRight' && hasNextDetail) {
        event.preventDefault();
  ...
  }, [detailSequence, hasNextDetail, hasPreviousDetail, selectedDetailIndex, viewMode]);
  ...
  const detailSequenceLabel = selectedInFiltered ? activeLensMeta.label : 'Canonical Timeline';
  const detailPositionLabel = selectedDetailIndex >= 0 ? `${selectedDetailIndex + 1} of ${detailSequence.length}` : undefined;
  const detailSequenceDisplay = detailPositionLabel ? `${detailSequenceLabel} (${detailPositionLabel})` : detailSequenceLabel;
  const isFilterActive = searchQuery.trim().length > 0 || activeLens !== 'all' || sortMode !== 'canonical';
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'dark' : ''}`}>
      <div className="min-h-screen bg-background text-foreground">
        ...
        <header className="sticky top-0 z-50 w-full border-b border-border/30 bg-background/95 backdrop-blur-md supports-[backdrop-filter]:bg-background/70">
          <div className="container flex h-16 items-center justify-between px-4 sm:px-6">
            <Link to="/" className="flex items-center gap-2 sm:gap-3 hover:opacity-80 transition">
              <Book className="h-5 w-5 text-primary shrink-0" aria-hidden="true" />
              <h1 className="font-serif text-base sm:text-lg md:text-xl font-medium tracking-tight leading-tight">History of the Western World</h1>
            </Link>
            <div className="flex items-center gap-2">
              ...
              <span className="caption text-muted-foreground hidden lg:block">From Creation to the Present Age</span>
              <Button
              ...
              >
                ...
                <span className="hidden sm:inline">{immersiveMode ? 'Immersive On' : 'Immersive'}</span>
              </Button>
              <Button
                variant="ghost"
                size="icon"
        ...
        <main id="main-content" className="container px-6 py-8 md:py-10" tabIndex={-1}>
          <div className="sr-only" aria-live="polite" aria-atomic="true">
            {viewMode === 'detail' && selectedPeriod ? `Viewing ${selectedPeriod.title}` : 'Viewing timeline'}
          </div>
          ...
          <AnimatePresence mode="wait">
            {viewMode === 'timeline' ? (
              ...
              >
                ...
                >
                  <p className="caption text-primary/80 mb-3 sm:mb-4">The Story of God&apos;s Dealings with Mankind</p>
                  ...
                  <p className="text-muted-foreground max-w-2xl mx-auto leading-relaxed mt-4 sm:mt-6 text-sm sm:text-base md:text-[1.08rem]">
                    Trace Scripture, civilizations, church history, and the American founding through primary sources.
                    Each era links to figures, definitions, and direct references so you can verify claims.
                  </p>
                  <div className="mt-6 sm:mt-8 flex items-center justify-center">
                    <div className="relative pl-5 sm:pl-6 max-w-xl text-left">
                      ...
                      <cite className="block mt-2 text-xs sm:text-sm text-primary font-semibold not-italic tracking-wide uppercase">Genesis 1:1</cite>
                    </div>
                  </div>
                </motion.div>
                ...
                {!immersiveMode && (
                  <section className="mb-8 rounded-2xl border border-border/40 bg-card/70 p-4 shadow-sm backdrop-blur-sm sm:p-5">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                      <div className="space-y-3 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-foreground">Browse by Era</span>
                        </div>
                        ...
                        <p className="text-xs text-muted-foreground hidden sm:block">{activeLensMeta.description}</p>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 sm:flex sm:items-center sm:gap-2.5">
                        ...
                        <Button variant="secondary" size="sm" onClick={handleSurpriseMe} disabled={filteredPeriods.length === 0} className="gap-1.5 h-11 sm:h-10 font-medium">
                          <Sparkles className="h-3.5 w-3.5" />
                          <span className="hidden xs:inline">Surprise Me</span>
                          <span className="xs:hidden">Random</span>
                        </Button>
                        <Button variant="ghost" size="sm" onClick={handleClearFilters} className="h-11 sm:h-10 hidden sm:flex">Clear</Button>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                      <Badge variant="secondary" className="px-2.5 py-1">{filteredPeriods.length} periods</Badge>
                      <Badge variant="outline" className="px-2.5 py-1">{totalVisibleFigures} figures</Badge>
                      <Badge variant="outline" className="px-2.5 py-1">{totalVisibleEvents} events</Badge>
                      <Badge variant="outline" className="px-2.5 py-1 hidden sm:inline-flex">~{estimatedVisibleMinutes} min</Badge>
                      <Badge variant="outline" className="px-2.5 py-1 hidden md:inline-flex">Press / to search</Badge>
                    </div>
                  </section>
                )}

                {filteredPeriods.length > 0 ? (
                  <ScrollArea className={immersiveMode ? 'h-[calc(100vh-23rem)]' : 'h-[calc(100vh-30rem)] md:h-[calc(100vh-29rem)]'}>
                    <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-6 list-none pb-6" aria-label="Historical periods">
                      {filteredPeriods.map((period, index) => {
                        const isFeatured = index % (activeLens === 'all' ? 5 : 4) === 0;
                        return (
                          <li key={period.id} className={`${isFeatured ? 'lg:col-span-6 lg:row-span-2' : 'lg:col-span-3'}`}>
                            <TimelineCard
                              period={period}
                              index={index}
                  ...
                  <div className="rounded-2xl border border-border/40 bg-card/80 p-7 text-center">
                    ...
                    <p className="text-sm text-muted-foreground mt-2">
                      Try different keywords, switch lens, or clear filters to return to the full timeline.
                    </p>
                    {isFilterActive && (
                      <Button variant="outline" className="mt-4" onClick={handleClearFilters}>Reset Explorer</Button>
                    )}
                  </div>
                )}
              </motion.div>
        ...
        <footer className="border-t border-border/30 py-8 mt-auto">
          <div className="container px-6 text-center">
            <p className="text-xs text-muted-foreground">
              Sourced from Scripture, primary documents, and freely accessible archives. No Wikipedia.
            </p>
            <p className="text-xs text-muted-foreground mt-2 font-serif italic">Fiat justitia, ruat caelum — Let justice be done, though the heavens fall.</p>
          </div>
        </footer>
      </div>
    </div>
